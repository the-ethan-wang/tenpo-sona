...
pygame.init()
WIDTH, HEIGHT = 24*board_width, 24*board_height
...
cant_think_of_a_good_variable_name_so_its_time_for_a_really_really_really_terrible_variable_name_because_its_really_long = True
board_state = [[random.choice([0, 1]) for i in range(board_width)] for i in range(board_height)]
t=0
...
while running:
    ...
    for i in range(board_height):
        for j in range(board_width):
            if(board_state[i][j]):
                rect = pygame.Rect(24*i, 24*j, 24, 24)
    ...
    if(t%speed_ratio==0 and cant_think_of_a_good_variable_name_so_its_time_for_a_really_really_really_terrible_variable_name_because_its_really_long):
        ...
        for i in range(board_height):
            for j in range(board_width):
                ...
                if(not board_state[i][j] and alive_neighbours in {3, 4}):
                    new_board[i][j] = 1
